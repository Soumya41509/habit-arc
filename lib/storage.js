...
export const getWaterLogs = async () => {
    try {
        const jsonValue = await AsyncStorage.getItem('water_logs');
        return jsonValue != null ? JSON.parse(jsonValue) : {};
    } catch (e) {
...
export const logWater = async (amount, date) => {
    try {
        ...
        await AsyncStorage.setItem('water_logs', JSON.stringify(logs));
        return logs;
    } catch (e) {
...
export const getWaterGoal = async () => {
    try {
        ...
        return goal ? parseInt(goal) : 2000; // 2000ml default
    } catch (e) {
